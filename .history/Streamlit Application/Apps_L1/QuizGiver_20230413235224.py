...
def app(QuizDoc, time_stamp):
    ...
    for Index in range(len(Ques)):
        st.write()
        st.write('-----------------------------------------------------------')
        st.session_state['Answer'] = st.radio(Ques[Index], Questionaire[Ques[Index]])
        Save = st.button("Save for Q " + str(Index + 1))
        if Save:
            st.session_state['AnswerList'][Index + 1] = [st.session_state['Answer'][0], time_stamp]
